...
> ⚠️ **selection 域已重构为兼容性规则引擎（CRE，2026-07-29）**：上表 5 个 selection type 不再存 `strategies` 表，改存独立表 `rules.compatibility_rules`（domain=selection），声明式 `WHEN(条件树)→THEN(动作)`，无序可叠加。前端求值抽到 `stores/selectionEngine.ts`（纯函数 + 28 单测），消费见工作台 `selectionActions`。默认种子 4 条、治理进度与**后续 roadmap（④ 后端兜底 / ⑤ filter 消费 / ⑥ 命中埋点 / ⑦ 规则补全 / ⑧ 寻址空间）**见文末「选型配置治理与后续 roadmap」。

### 场景化匹配（报价策略重构后）

pricing 域采用 **场景 + 规则 + 连线** 模型（替代旧的 margin_tier 三维 scope 直匹配）：
- `pricing_scenario`（场景）：带 scope（platform_type / customer_type / quote_scenario 三维，留空 = 通用兜底）+ `body.rule_id` 连线到规则
...
### 默认种子（4 条 · `compatibility_rule_repo.DEFAULT_RULES`）

| # | type | status | 说明 |
|---|------|--------|------|
| ① | filter | active | 按商机 `platform_type` 过滤候选机型 — ⚠️ **无执行消费端**（产出只变 info 文字，不驱动候选过滤，见 ⑤） |
| ② | exclude | active | 内存同型号不混搭（`kp.Memory` ≥2 条，`unique_field=pn` 唯一） |
| ③ | require | testing | 选 GPU 需配 GPU 电源线 — 料号库件不进 CRE 寻址空间（见 ⑧），约束已由后端 `derive_gpu_cables` 承担 |
| ④ | require | testing | NVMe 盘需配 tri-mode 背板 — 同上，背板类型由后端 `derive_bp_type` 推导 |

> ⚠️ **线上 DB 仍为旧版 4 条 active（零自定义、hit_count 全 0）**：治理②调整的 status（③④ → testing）需 `POST /api/compatibility-rules/reset` 同步（无损，因无自定义规则）。
...
- **④ 后端兜底验证（推荐方案 B，待时机再加）**
  ...
  - **可安全推迟的理由**：选型规则当前是建议层（只警告不锁）；高风险派生（功耗/PSU/线缆/背板）已由后端 `DerivationEngine` 承担，无致命缺口；前端单测保正确性下限。
...
- **⑧ CRE 寻址空间扩展（架构决策 · 用户拍板）**：`ctx` 只聚合 `category='Key Parts'` 的件（`kp.*/config.*/opportunity.*`），料号库件（线缆/背板/电源）不进寻址。二选一：**扩 ctx** 纳入料号库件（激活 ③④）／**维持分工**——前端 CRE 管 KP 互斥/必配 + 后端 `DerivationEngine` 管功耗/线缆/背板（③④ 保持 testing）。
