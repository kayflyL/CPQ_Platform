...
4. **右栏：定价与利润** — 实时计算：
   ...
   - 售价计算
   - 利润率显示
   - **利润率告警**：综合毛利率低于 `system_config.profit_margin_alert_threshold`（默认 8%）时弹一次性 Modal 提示线下特价审批；阈值走 `usePricingRules` composable，不写死
   - 价格参数调整
...
| POST | `/api/quotations/{id}/export` | `quotationApi.export` | 导出动作：冻结草稿为已导出（盖 exported_at + 落 cost_snapshot） |
...
- **导出即冻结**：点「下载 Excel」后，前端从 store（`getConfigTotals` + `l6_section_totals` + 汇率/税率）组装 cost_snapshot 并 POST `/quotations/{id}/export`，后端盖 `exported_at` + 落库。冻结后该报价单不再进工作台（在详情页点开只看成本快照 + Excel），编辑需「重新解析」生成新草稿。冻结比实时重算更准——反映导出当时的价格。
