...
async def export_opportunity(opportunity_id: str, template_id: str = None):
    """Export opportunity, save to storage, and return file for download"""
    service = QuoteService()
    try:
        output, filename = service.export_opportunity(opportunity_id, template_id)
        if output is None:
            raise HTTPException(status_code=404, detail="Project not found")
        ...
        if not folder_name:
            # Generate folder_name if not exists (for backward compatibility)
            file_storage = FileStorage()
            folder_name = file_storage.generate_opportunity_folder_name(opportunity_info)
            # Update database with folder_name
            opportunity_repo = OpportunityRepository()
        ...
        # Save exported file to opportunity folder
        file_storage = FileStorage()
        file_info = file_storage.save_export(folder_name, file_content, filename)
...
@router.post("/{opportunity_id}/preview")
async def preview_opportunity(opportunity_id: str, template_id: str = None):
    """Preview opportunity export (generate Excel without saving to storage)"""
    service = QuoteService()
    try:
        output, filename = service.export_opportunity(opportunity_id, template_id)
        if output is None:
...
@router.post("/{opportunity_id}/preview-json")
async def preview_opportunity_json(opportunity_id: str, template_id: str = None):
    """Preview opportunity export as JSON (SheetRenderData[] format for frontend ExcelTable)"""
    import io
    ...
    try:
        ...
        # Load the generated Excel into openpyxl
        wb = load_workbook(io.BytesIO(output.getvalue()), data_only=True)
        
        sheets_data = []
        
        for ws in wb.worksheets:
            # Build merge map: (row, col) -> MergeRegion
            merge_map = {}
            merges = []
                ...
                for c in range(1, max_col + 1):
                    ...
                    if cell.font:
                        ...
                        if cell.font.color and cell.font.color.rgb:
                            style["font"]["color"] = {"argb": str(cell.font.color.rgb)}
                    
                    if cell.fill and cell.fill.patternType and cell.fill.patternType != 'none':
                        if cell.fill.fgColor and cell.fill.fgColor.rgb:
                            rgb_str = str(cell.fill.fgColor.rgb)
                            # Skip default/transparent colors (avoid black background)
                            if rgb_str and rgb_str not in ('00000000', '0', 'FFFFFFFF'):
                                style["fill"] = {"fgColor": {"argb": rgb_str}}
                    ...
                    if cell.alignment:
                        ...
                        if cell.alignment.wrap_text:
                            style["alignment"]["wrapText"] = cell.alignment.wrap_text
                    ...
                    if cell.border:
                        ...
                        for side in ["top", "left", "bottom", "right"]:
                            ...
                            if border_side and border_side.style:
                                style["border"][side] = {"style": border_side.style}
                                if border_side.color and border_side.color.rgb:
                                    style["border"][side]["color"] = {"argb": str(border_side.color.rgb)}
                    
                    if cell.number_format and cell.number_format != 'General':
                        style["numFmt"] = cell.number_format
                    ...
                    # Handle merge
                    if (r, c) in merge_map:
                        merge_info = merge_map[(r, c)]
                        cell_data["merge"] = merge_info["region"]
                        if merge_info["isPrimary"]:
                            cell_data["rowSpan"] = merge_info["rowSpan"]
                            cell_data["colSpan"] = merge_info["colSpan"]
                        else:
                            cell_data["isMerged"] = True
